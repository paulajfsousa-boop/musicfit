...
let lessonTracks = [], activeLessonIndex = -1, lessonPlaying = false, switchingTrack = false, playerPaused = true;
...
function fmtClock(t){if(!isFinite(t))return'0:00';const m=Math.floor(t/60);const s=Math.floor(t%60).toString().padStart(2,'0');return m+':'+s;} function updateSeekUI(){if(!draggingSeek){seekBar.max=Math.max(0,currentDuration||0);seekBar.value=Math.max(0,Math.min(currentPosition||0,currentDuration||0));}document.getElementById('seekTime').textContent=fmtClock(currentPosition||0)+' / '+fmtClock(currentDuration||0);} setInterval(async()=>{if(player){const st=await player.getCurrentState().catch(()=>null);if(st){currentPosition=st.position/1000;currentDuration=st.duration/1000;updateSeekUI();updateRun();if(lessonPlaying&&!switchingTrack&&currentDuration>0&&(currentDuration-currentPosition)<=0.35){advanceLessonTrack();}}}},250);
function fmt(t){const m=Math.floor(t/60),s=Math.floor(t%60).toString().padStart(2,'0'),d=Math.floor((t%1)*10);return `${m}:${s}.${d}`;} function esc(s){return(s||'').replace(/[&<>"']/g,m=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#039;'}[m]));}